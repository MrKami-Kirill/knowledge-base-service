...
public record MenuItemDataDto(
    @Schema(description = "Название элемента меню", example = "Страница 1", requiredMode = REQUIRED)
    String title,
    @Schema(description = "Есть ли дочерние элементы у меню", example = "true",
        requiredMode = REQUIRED)
    Boolean hasChildren,
    @Schema(description = "Путь до элемента меню", example = "/stranica_1", requiredMode = REQUIRED)
    String path,
